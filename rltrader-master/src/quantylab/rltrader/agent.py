...
class Agent:
    ...
    def decide_trading_unit(self, confidence):  # 정책 신경망이 결정한 행동의 신뢰가 높을수록 매수 또는 매도하는 단위를 크게
        ...
        added_trading_price = max(min(
            ...
            self.max_trading_price-self.min_trading_price), 0)  # 신뢰도*(최대 단일 매매 금액 - 최소 단일 매매 금액)와 (최대 단일 매매 금액 - 최소 단일 매매 금액) 중에 작은 값과 0 중에 큰 값
        trading_price = self.min_trading_price + added_trading_price  # 거래 가격 = 최소 단일 매매 금액 + 추가 가격
